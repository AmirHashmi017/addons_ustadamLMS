...
class WebsiteBlog(http.Controller):
    ...
    def _prepare_blog_values(self, blogs, blog=False, date_begin=False, date_end=False, tags=False, state=False,
                             page=False, search=None, is_Fund=False, is_News=False, **post):
        ...
        if blog:
            ...
            if is_Fund:
                # When is_Fund is True, filter only 'Investment Funding' and 'Donations Funding'
                domain += [('blog_id.name', 'in', ['Investment Funding', 'Donations Funding'])]
            if is_News:
                # When is_News is True, filter only 'News' blog
                domain += [('blog_id.name', '=', 'News')]
            else:
                # Exclude 'News' blog when not specifying a blog or Fund filter
                domain += [('blog_id.name', 'not in', ['News', 'Investment Funding', 'Donations Funding'])]
        ...
        if is_Fund:
            # When is_Fund is True, filter only 'Investment Funding' and 'Donations Funding' and update total
            posts = posts.filtered(lambda p: p.blog_id.name in ['Investment Funding', 'Donations Funding'])
            total = len(posts)
        elif is_News:
            # When is_News is True, filter only 'News' blog and update total
            posts = posts.filtered(lambda p: p.blog_id.name == 'News')
            total = len(posts)
        # Filter out posts belonging to the "News" blog if no specific blog is selected and is_Fund is False
        elif not blog:
            posts = posts.filtered(lambda p: p.blog_id.name not in ['News', 'Investment Funding', 'Donations Funding'])
            total = len(posts)
    ...
    @http.route([
        ...
        '/blog/news',
        '/blog/fund',
        '/blog/fund/page/<int:page>',
        '/blog/fund/<model("blog.blog"):blog>',
    ], type='http', auth="user", website=True, sitemap=True)
    def blog(self, blog=None, tag=None, page=1, search=None, is_News=False, is_Fund=False, **opt):
        Blog = request.env['blog.blog']

        # Adjust search domain based on `is_News` and `is_Fund`
        search_domain = []
        if request.httprequest.path == '/blog/news':
            is_News = True
            search_domain = [('name', '=', 'News')]
        elif request.httprequest.path.startswith('/blog/fund'):
            is_Fund = True
            search_domain = [('name', 'in', ['Investment Funding', 'Donations Funding'])]
        else:
            search_domain = [('name', 'not in', ['News', 'Investment Funding', 'Donations Funding'])]

        # Fetch blogs based on the search domain
        blogs = Blog.search(search_domain, order="create_date asc, id asc")

        # Redirect to the first blog if none is provided and blogs exist
        if not blog and len(blogs) == 0:
            url = QueryURL('/blog/%s' % slug(blogs[0]), search=search, **opt)()
            return request.redirect(url, code=302)

        date_begin, date_end = opt.get('date_begin'), opt.get('date_end')

        # Handle multiple tags in the URL
        ...
        values = self._prepare_blog_values(blogs=blogs, blog=blog, tags=tag, page=page, search=search, is_Fund=is_Fund,
                                           is_News=is_News,
                                           **opt)
        values['is_Fund'] = is_Fund
        values['is_News'] = is_News

        # Check for redirection if needed
        if isinstance(values, werkzeug.wrappers.Response):
            return values

        # Set the main object if blog exists
        if blog:
